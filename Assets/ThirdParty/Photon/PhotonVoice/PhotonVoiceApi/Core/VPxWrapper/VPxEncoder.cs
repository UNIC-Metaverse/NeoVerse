...
namespace VPx.Encoder
{
    ...
    {
        public const int VPX_ENCODER_ABI_VERSION = 14 + VPxConst.VPX_CODEC_ABI_VERSION;
    ...
    {
        IntPtr buf; /* Pointer to the data */                         // void *
        UIntPtr /*size_t*/ sz;  /* Length of the buffer, in chars */  // size_t
    }
    ...
    public struct vpx_codec_cx_pkt_t
    {
        ...
        public struct data_t
        {
            ...
            {
                public IntPtr buf;      /* compressed data buffer */                          // void *
                public UIntPtr sz;       /* length of compressed data */                      // size_t
                public Int64 pts;      /* time stamp to show frame (in timebase units) */     // int64_t
                public
// uint is 32 bit on both 32 and 64 bit Windows.
// We should use uint for all 32 bit platforms but there is no way to detect 32 bit here
// because exported projects are mostly multi-platform (UNITY_64 is not set for them, UNITY_EDITOR_64 helps in editor only).
// So we support 32 bit platform on Windows only.
// Wrong type usage usually results in 'flag' field is always 0 and Android remote client can't start decoding while waiting for the keyframe
#if PHOTON_VOICE_WINDOWS || UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
                        uint
#else
                        ulong
#endif
                                duration; /* duration to show frame (in timebase units) */    // unsigned long
                public uint flags;    /* flags for this frame */                              // uint32_t
                public int partition_id; /* the partition id ... */                           // int
        ...
        public vpx_codec_cx_pkt_kind kind; /* packet variant */                               // enum vpx_codec_cx_pkt_kind
        public data_t data;
    }
    // sizeof = 384
    public struct vpx_codec_enc_cfg
    {
         ...
         */
        public int rc_2pass_vbr_corpus_complexity;
        /*
